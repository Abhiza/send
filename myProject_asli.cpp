...
float speed = 0.3;          // setup speed motor
float steering = 20.0;      // steering with offset 20 degree
...
extern "C" void* myProgram2(void* p) // untuk logging kecepatan sudut yang perintah, output .csv, di stm ada sendiri pake laptop, susah wkwkwkwk.
{	
	while(mobiljalan==false);
	if(mobiljalan==true)
	{
    try
    {
        csvfile csv("logging.csv");
        csv << "ms" << "speed" << "steering" << endrow;
        ...
        while(true)
        {
            if(get_millis()-ms3>=500)
            {
                ...
                csv << myBuffer << speed << steering-20.f << endrow;
            }
        }
    }
    catch(const std::exception& e)
